# -*- coding: utf-8 -*-
"""Live forwarding and bounded, account-isolated exhibition data channels.
...
def forward(kind, context):
    cfg = config()
    if kind in ("scene_intent", "scene_invoke"):
    ...
    try:
        ...
        if parsed.scheme not in ("http", "https") or not parsed.hostname or parsed.username or parsed.password or parsed.fragment:
            raise ValueError()
        body = _template(route.get("body", {"text": "$text"} if kind in ("intent", "scene_intent") else "$arguments"), context)
    ...
    try:
        ...
        with httpx.Client(trust_env=False, follow_redirects=False, timeout=timeout) as client:
            with client.stream(method, endpoint, json=body, headers=headers) as response:
...
def owner(channel_id, account):
    with LOCK:
        channel = CHANNELS.get(channel_id)
        if not channel or channel["owner"] != account:
...
async def read_event(request):
    try:
        event = json.loads(await bounded_body(request, MAX_JSON))
    except (ValueError, UnicodeDecodeError):
        raise HTTPException(422, "需要 JSON 对象")
    return event
...
def mount_routes(app, auth):
    ...
    def channels(authorization: str = Header(None)):
        ...
        with LOCK:
            return {"channels": [{"id": cid, "name": c["name"], "event_count": len(c["events"]), "service_id": c.get("service_id")}
                                  for cid, c in CHANNELS.items() if c["owner"] == rec["account"]]}
    ...
    def feedback_access(service_id: str, response: Response, authorization: str = Header(None)):
        """Owner-only, idempotent provisioning; colleagues only use scene-feedback + Key."""
        _, rec = auth(authorization)
        response.headers["Cache-Control"] = "no-store"
    ...
    def events(channel_id: str, authorization: str = Header(None),
        ...
        with LOCK:
            ...
            selected = [event for event in retained if event["id"] > cursor
                        and (request_id is None or event.get("request_id") == request_id)]
            return {"channel_id": channel_id, "name": channel["name"], "events": selected,
                    "next_cursor": channel["sequence"], "reset_required": reset,
