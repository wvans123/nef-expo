...
分类：运行手册。更新：2026-09-20。当前展示范围以本节为准，后面的旧工作台流程仅供历史核对。
...
1. **能力超市**：场景套餐与基础能力并列供给，卡内显示组成与折扣月价，点击能力复用详情弹窗。TRF / ARF 目录由页面自动拉取；顶部“发布 NEF 目录”保留选项、预览和显式发布，下方展示网络导入、自建套餐及 AF 能力源 / 工具。
...
**下一次加载前：** 将已约定地址填入 `config/integration.local.json`，字段和路径见 [curl 手册](reference/manual-curl.md)；公开文档不收录现场 IP，已有 local 文件不会自动合并新模板。与操作者约定重启窗口及内存账号/订阅重新准备方式后再加载。启动清理农场套餐默认关闭，需明确设置 `reset_partner_plans_on_start:true` 才执行。新代码尚未在本机获得真实伙伴接收回执，未验证 PA/CA 执行。
