...
5. **Intent 主流程**：三个场景都可从套餐进入 Intent。填写业务目标，运行明确标注的场景演示，展示访问核验与文字结果；需要时再展开图片 / 视频。端网协同保留 API / Tool 作为协议对照，不再限制为这两种模式。
6. **MCP 北向接口**：仍从“连接 NEF 并发现工具 → 选择工具 → tools/call”开始。它是 AF 发现 NEF 的工具，不能与双向开放中“NEF 发现外部 MCP Server”的方向混淆。
7. **真实联调**：切换真实调用，NEF 原文转发至内部接口，呈现对方实际文字；长任务从统一回传接口送入结果。没有内部任务 ID 也可回传，但不能把未关联的场景反馈认定为某次 Intent 完成。
...
主流程已调整为农场页面跳转 `https://nef.2012wtlab.com/?account_id=1` → 用户订购 → NEF POST `subscriberId` + `servicePlan` 到农场 `/business/v1/service-plans`。选中能力才附 `servicePlan.networkCapabilities`；不选则省略，PA 自主编排由网络侧实现。详细字段、配置和重试唯一维护于[套餐订购通知与订阅查询](reference/subscription-query.md#10-跳转订购与套餐通知)。
...
2026-09-17 最新代码验证：244 项 Python 测试和 5 组 Node 脚本通过，保留 3 条依赖弃用警告。包括嵌套订购通知、subscriberId、精确所选能力、空选择省略字段、价格缺失、失败不撤销权益、同事件同正文重试、查询、场景回传及目录发布。真实回环 HTTP 测试对端均为模拟服务，不联系真实农场。独立临时 NEF + 无界面 Edge 验证跳转账号、选择/不选能力的实际订购请求、提示状态和桌面/390px 手机弹窗；测试进程均已停止，未操作 8069 的账号。
...
**下一次加载前：** 等同事提供回调完整地址、鉴权和成功响应，双方确认套餐价格，填写 `config/subscription.local.json`。与操作者约定重启窗口及内存账号/订阅重新准备方式后再加载；不能为了部署通知而打断当前查询联调。新代码没有收到真实同事 HTTP 接收回执，更没有验证 PA/CA 已执行约束。
...
**协议与反馈，约 1 分钟。** 按需补充 MCP 发现优先和 API 已知接口的区别。切换真实调用后自动准备场景接口，打开“接口信息”即可交付地址和 Key，不创建或选择调用点。场景同事只拿一个回传地址和 Key，既能回文字，也能回图像 / 视频。服务重启后需要重新获取 Key。
...
```powershell
python -m uvicorn server:app --host 0.0.0.0 --port 8069
```
...
- 将 `config/registry.example.json` 复制为本机配置；设置 `NEF_REGISTRY_CONFIG` 为其绝对路径。
