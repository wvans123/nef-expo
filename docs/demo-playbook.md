...
2026-09-22 20:19:46 通过 Windows 任务 `NEF-Expo` 重启，改为按 nf tool、computing tool、sensing tool 登记三个 MCP Server，url 暂用 NEF 自身根地址。监听 PID `11348`，父进程 `svchost.exe`，实例 `ae4a657231095e00`，任务 Running；首页返回 200，正式接口确认 Server 数为 3、覆盖能力为 23。实际 TRF 地址仍空，三个状态为 unknown；没有把本机验证说成现场注册成功，也未修改本地配置。重启清空了演示账号和订阅。

69 项定向测试通过（TRF/单能力 MCP 52 项、共享网络登记 17 项），未跑全量。桌面夹具真实发送三条 POST，并用下划线字段、改写描述和尾斜杠的 GET 验证“已注册 3 个、覆盖能力 23 项”；还覆盖重复同步不重发、重启缓存恢复撤回、GET 失败显示待确认、旧逐能力记录与第三方隔离。单元用例另覆盖 GET 缺少描述/状态及运行状态变化。8071 夹具已停止。旧条目只在显式取消时撤回可核对归属的记录，同步不会自动删除。真实问题仍待对端 GET 回包确认。

### 同日 17:16 部署与验收快照（已被三分类登记替代）
