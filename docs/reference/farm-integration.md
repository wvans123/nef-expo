...
分类：接口参考 / 联调流程。更新：2026-09-22。面向农场平台、NEF 和网络侧开发同事；巡检小车使用相同 MCP 接入流程。
...
- 购买场景时勾选的子能力会同步获得独立 API / MCP 权益，在 `tools[].grant_sources` 中标记 `scene:<id>`；未选组件不因该场景获权。优先用 tools 的准确工具名和 schema，不能仅凭套餐的组件描述拼调用参数。
